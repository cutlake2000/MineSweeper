...
public class Board
{
    ...
    public Board(int mapX, int mapY, MineSpawner mineSpawner)
    {
        ...
        _tiles = TileManager.CreateTiles(mapX, mapY);
    ...
    public int RaiseFlagOnTile(int x, int y)
    {
        ...
        return ChangeTileState(tile, TileState.Flag);
    }
    
    /// <summary>
    /// 타일의 상태를 변경하고, 점수에 반영할 값을 리턴합니다.
    /// </summary>
    /// <param name="tile">타일 객체</param>
    /// <param name="newState">변경할 상태</param>
    /// <returns>현재 타일 상태에 따른 점수 업데이트 값</returns>
    private int ChangeTileState(Tile tile, TileState newState)
    {
        if (tile.CurrentTileState == newState) return 0;

        // 깃발을 꽂거나 제거할 때 점수 변동 사항을 처리합니다.
        switch (tile.CurrentTileState)
        {
            case TileState.Close when newState == TileState.Flag:
                tile.ChangeState(newState);
                return tile.IsMine ? 1 : -1;
            
            case TileState.Flag when newState == TileState.Close:
                tile.ChangeState(newState);
                return tile.IsMine ? -1 : 1;
        ...
        while (queue.Count > 0)
        {
            ...
            if (tile.NearbyMineCount == 0)
            {
                foreach (var nearby in TraverseNearbyTiles(posX, posY))
                {
                    ...
                    if (nearbyTile.CurrentTileState == TileState.Close)
                    {
